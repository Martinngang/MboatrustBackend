// Real, end-to-end proof of the Quincaillerie materials-delivery system:
// genuine HTTP requests against a running server, covering the Project
// materials-choice fields, the full MaterialOrder lifecycle, and the
// permission boundaries for both required paths — Funder → Quincaillerie →
// Project and Contractor → Quincaillerie → Project. Mirrors
// verifyProjectTenderRoleSeparation.js / verifyQuincaillerieProfile.js's
// shape. Safe to re-run any time: every fixture is tagged and deleted again
// at the end, pass or fail.
const axios = require('axios');
const { connectDB } = require('../config/db');
const mongoose = require('mongoose');
const { User, Project, Bid, QuincaillerieProfile, MaterialOrder, Escrow } = require('../models');
...
async function run() {
  ...
  try {
    ...
    const storeOwner = await User.create({
      ...
      firebaseUid: `${TAG}-store-${Date.now()}`, roles: [{ roleType: 'quincaillerie' }],
    });
    const otherStoreOwner = await User.create({
      fullName: `${TAG} Other Store Owner`, email: `${TAG}-other-store-${Date.now()}@test.local`,
      firebaseUid: `${TAG}-other-store-${Date.now()}`, roles: [{ roleType: 'quincaillerie' }],
    });
    createdUserIds.push(
    ...
    // ── Set up an approved quincaillerie and a still-pending one ──────────
    const storeReg = {
    ...
    const storeSubmit = await storeClient.post('/quincaillerie-profiles/me', storeReg);
    const storeId = storeSubmit.data.data._id;
    await adminClient.post(`/quincaillerie-profiles/${storeId}/approve`);

    const pendingStoreSubmit = await otherStoreClient.post('/quincaillerie-profiles/me', { ...storeReg, businessName: `${TAG} Pending Store` });
    ...
    record('New tender defaults to contractor-managed materials', tender.data.data.materialsManagedBy === 'contractor' && tender.data.data.preferredQuincaillerieId === null);
    const projectId = tender.data.data._id;
    const milestoneId = tender.data.data.milestones[0]._id;
    createdProjectIds.push(projectId);

    // ── Assign-quincaillerie: the "browse and assign later" flow ─────────
    const strangerAssign = await strangerClient.post(`/projects/${projectId}/assign-quincaillerie`, { quincaillerieId: storeId });
    ...
    const assignPending = await funderClient.post(`/projects/${projectId}/assign-quincaillerie`, { quincaillerieId: pendingStoreId });
    record('Cannot assign a not-yet-approved quincaillerie', assignPending.status === 400, `got ${assignPending.status}`);

    const assign = await funderClient.post(`/projects/${projectId}/assign-quincaillerie`, { quincaillerieId: storeId });
    record(
      'Funder can assign an approved supplier after creation',
      assign.status === 200 && assign.data.data.materialsManagedBy === 'quincaillerie' && assign.data.data.preferredQuincaillerieId === storeId
    );

    const unassign = await funderClient.post(`/projects/${projectId}/assign-quincaillerie`, { quincaillerieId: null });
    record(
      'Funder can unassign, reverting to contractor-managed',
      unassign.status === 200 && unassign.data.data.materialsManagedBy === 'contractor' && unassign.data.data.preferredQuincaillerieId === null
    );

    // Re-assign for the rest of the script's order-lifecycle checks below.
    await funderClient.post(`/projects/${projectId}/assign-quincaillerie`, { quincaillerieId: storeId });

    // ── Award the contractor (needed for the Contractor → Quincaillerie path) ──
    ...
    const orderPayload = (quincaillerieId) => ({
      projectId, milestoneId, quincaillerieId,
    ...
    record('Cannot request materials from a not-yet-approved quincaillerie', pendingStoreCreate.status === 400, `got ${pendingStoreCreate.status}`);

    // Funder → Quincaillerie → Project.
    const funderOrder = await funderClient.post('/material-orders', orderPayload(storeId));
    record('Funder (project owner) can request materials — Funder → Quincaillerie → Project', funderOrder.status === 201, `status=${funderOrder.status}`);
    record(
      'Order response is enriched with display fields (store/project/requester names)',
      funderOrder.data.data.quincaillerieId.businessName === storeReg.businessName
    ...
    // Contractor → Quincaillerie → Project.
    const contractorOrder = await contractorClient.post('/material-orders', orderPayload(storeId));
    record('Awarded contractor can request materials — Contractor → Quincaillerie → Project', contractorOrder.status === 201, `status=${contractorOrder.status}`);
    ...
    const storeQueue = await storeClient.get('/material-orders/for-quincaillerie');
    record(
    ...
    const otherStoreQueue = await otherStoreClient.get('/material-orders/for-quincaillerie');
    ...
    const beforeCount = (await QuincaillerieProfile.findById(storeId).lean()).completedOrderCount;
    ...
    const afterCount = (await QuincaillerieProfile.findById(storeId).lean()).completedOrderCount;
    ...
    // routes the disbursement to the quincaillerie's own payout details
    // instead of the project's usual payee.
    ...
    record(
      'Release escrow routes to the quincaillerie, not the usual payee',
      releasedEscrow?.payeeType === 'quincaillerie' && String(releasedEscrow?.payeeQuincaillerieId) === String(storeId),
      `payeeType=${releasedEscrow?.payeeType}`
    );
    record(
      'Release uses the quincaillerie\'s own payment provider/payout number, not a null/default one',
    ...
    await QuincaillerieProfile.deleteMany({ ownerId: { $in: createdUserIds } });
