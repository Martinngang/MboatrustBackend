// Real, end-to-end proof that the quincaillerie application/approval
// pipeline works — genuine HTTP requests against a running server. Mirrors
...
// recipients browse this; verifier profiles have no equivalent), and the
// generic admin role-grant/revoke drawer (GRANTABLE_ROLES) also being able
// to reach 'quincaillerie' directly, not just via the review queue.
...
const { User, QuincaillerieProfile } = require('../models');

const TAG = 'verify-quincaillerie-profile-script';
...
async function run() {
  ...
  try {
    ...
    // ── Self-submission ──────────────────────────────────────────────
    const submit = await applicantClient.post('/quincaillerie-profiles/me', registration);
    ...
    record('No role granted yet on submission', !afterSubmit.roles.some((r) => r.roleType === 'quincaillerie'));

    // Not visible in the public directory (pending, not approved).
    const dirBeforeApproval = await applicantClient.get('/quincaillerie-profiles/directory');
    ...
    const nonAdminApprove = await outsiderClient.post(`/quincaillerie-profiles/${profileId}/approve`);
    ...
    const queue = await adminClient.get('/quincaillerie-profiles', { params: { applicationStatus: 'pending' } });
    record('Admin review queue lists the pending registration', queue.status === 200 && queue.data?.data?.some((p) => p._id === profileId));

    // ── Approval — atomicity ──────────────────────────────────────────
    const approve = await adminClient.post(`/quincaillerie-profiles/${profileId}/approve`);
    record(
    ...
    record('Role is actually on the user immediately after approve', afterApprove.roles.some((r) => r.roleType === 'quincaillerie'));

    // Now visible in the public directory.
    const dirAfterApproval = await outsiderClient.get('/quincaillerie-profiles/directory');
    record('Approved registration now appears in the public directory', dirAfterApproval.data?.data?.some((p) => p._id === profileId));

    // ── Generic admin role drawer can also reach 'quincaillerie' ──────
    const revoke = await adminClient.delete(`/admin/users/${applicant._id}/roles/quincaillerie`);
    record('Admin can revoke the quincaillerie role via the generic role endpoint', revoke.status === 200 && !revoke.data?.data?.roles?.some((r) => r.roleType === 'quincaillerie'));
    const regrant = await adminClient.post(`/admin/users/${applicant._id}/roles`, { roleType: 'quincaillerie' });
    record('Admin can grant the quincaillerie role via the generic role endpoint', regrant.status === 200 && regrant.data?.data?.roles?.some((r) => r.roleType === 'quincaillerie'));
    const selfGrantAttempt = await applicantClient.post('/users/me/roles', { roleType: 'quincaillerie' });
    record('quincaillerie is never self-grantable (trust-elevating, admin/approval only)', selfGrantAttempt.status === 400, `got ${selfGrantAttempt.status}`);
    ...
    const submit2 = await applicant2Client.post('/quincaillerie-profiles/me', { ...registration, businessName: `${TAG} Store Two` });
    const profile2Id = submit2.data.data._id;
    const rejectRes = await adminClient.post(`/quincaillerie-profiles/${profile2Id}/reject`);
    record('Admin can reject a registration', rejectRes.status === 200 && rejectRes.data?.data?.applicationStatus === 'rejected');

    const reapply = await applicant2Client.post('/quincaillerie-profiles/me', { ...registration, businessName: `${TAG} Store Two Resubmitted` });
    ...
    record('Rejected applicant never got the role', !afterReject.roles.some((r) => r.roleType === 'quincaillerie'));
